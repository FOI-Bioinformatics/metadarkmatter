...
class ScoringConfig(BaseModel):
    ...
    Attributes:
        ...
        novelty_novel_species_max: Maximum novelty index for novel species (default: 15.0)
        novelty_novel_genus_min: Minimum novelty index for novel genus (default: 15.0)
...
class Bowtie2Config(BaseModel):
    ...
    - Local alignment mode handles partial matches common in divergent reads
    - Very-sensitive preset maximizes sensitivity for 75-95% identity sequences
    - High max_alignments captures all competing hits for placement uncertainty

    Attributes:
        num_threads: Number of CPU threads (default: 4)
        mode: Alignment mode - local or end-to-end (default: local)
        very_sensitive: Use very-sensitive preset for divergent sequences (default: True)
        max_alignments: Maximum alignments per read (-k parameter, default: 500)
    """

    num_threads: int = Field(default=4, ge=1, description="Number of CPU threads")
    mode: Literal["local", "end-to-end"] = Field(
        default="local",
        description="Alignment mode (local recommended for metagenomic reads)",
    )
    very_sensitive: bool = Field(
        default=True,
        description="Use very-sensitive preset for divergent sequences",
    )
    max_alignments: int = Field(
        default=500,
        ge=1,
        description="Maximum alignments per read (high value for complete competitive alignment)",
    )
...
class KrakenConfig(BaseModel):
    ...
    Attributes:
        ...
        minimum_base_quality: Minimum base quality for reads (default: 0)
    """

    database: Path = Field(description="Path to Kraken2 database")
    num_threads: int = Field(default=4, ge=1, description="Number of CPU threads")
    ...
    minimum_base_quality: int = Field(
        default=0,
        ge=0,
    ...
    def validate_database_exists(cls, v: Path) -> Path:
        """Ensure Kraken2 database directory exists."""
        if not v.exists():
            msg = f"Kraken2 database not found: {v}"
            raise ValueError(msg)
        return v
...
class ExtractConfig(BaseModel):
    """
    Configuration for read extraction from Kraken2 output.

    Attributes:
        include_children: Include reads from child taxa (default: True)
        confidence: Kraken2 confidence threshold (default: 0.0)
    """

    include_children: bool = Field(
        default=True,
        description="Include reads from child taxa",
    )
    confidence: float = Field(
        default=0.0,
        ge=0,
        le=1,
        description="Kraken2 confidence threshold",
    )
...
class MapConfig(BaseModel):
    """
    Configuration for competitive read mapping.

    Optimized for placement uncertainty calculation:
    - High max_alignments captures all competing hits
    - Very-sensitive preset for divergent sequence detection

    Attributes:
        mode: Alignment mode - local or end-to-end (default: local)
        very_sensitive: Use very-sensitive preset (default: True)
        max_alignments: Maximum alignments per read (default: 500)
        no_unal: Exclude unaligned reads from output (default: True)
        no_discordant: Exclude discordant paired-end alignments (default: True)
    """
    ...
    no_unal: bool = Field(
        default=True,
        description="Exclude unaligned reads from output",
    )
    no_discordant: bool = Field(
        default=True,
        description="Exclude discordant paired-end alignments",
    )
...
class VisualizeConfig(BaseModel):
    """
    Configuration for recruitment plot visualization.

    Attributes:
        min_identity: Minimum percent identity to display (default: 70.0)
        max_points: Maximum points to plot (default: 100000)
        known_species_threshold: Identity threshold for known species (default: 98.0)
        novel_species_threshold: Identity threshold for novel species (default: 85.0)
        novel_genus_threshold: Identity threshold for novel genus (default: 75.0)
    """
    ...
    min_identity: float = Field(
        ...
        ge=0,
        le=100,
        description="Minimum percent identity to display",
    )
    max_points: int = Field(
        default=100000,
        ge=1000,
        description="Maximum points to plot",
    )
    known_species_threshold: float = Field(
        default=98.0,
        ge=0,
        le=100,
        description="Identity threshold for known species",
    )
    novel_species_threshold: float = Field(
        default=85.0,
        ge=0,
        le=100,
        description="Identity threshold for novel species",
    )
    novel_genus_threshold: float = Field(
        default=75.0,
        ge=0,
        le=100,
        description="Identity threshold for novel genus",
    )
...
class GlobalConfig(BaseSettings):
    ...
    This uses pydantic-settings to load configuration from:
    1. YAML config file (if provided)
    2. Environment variables with MDM_ prefix
    3. CLI arguments (override all)

    Attributes:
        project_name: Name of the project/sample
        output_dir: Directory for all output files
        scoring: Scoring threshold configuration
        blast: BLAST tool configuration
        bowtie2: Bowtie2 tool configuration
        kraken: Kraken2 tool configuration (optional)
        num_threads: Global default for number of threads
    ...
    project_name: str = Field(
        default="metadarkmatter_analysis",
        description="Project/sample name",
    )
    ...
    scoring: ScoringConfig = Field(
        ...
        description="Scoring threshold configuration",
    )
    blast: BlastConfig = Field(
        default_factory=BlastConfig,
        description="BLAST configuration",
    )
    bowtie2: Bowtie2Config = Field(
        default_factory=Bowtie2Config,
        description="Bowtie2 configuration",
    )
    kraken: KrakenConfig | None = Field(
        default=None,
        description="Kraken2 configuration (optional)",
    )
    extract: ExtractConfig = Field(
        default_factory=ExtractConfig,
        description="Read extraction configuration",
    )
    map: MapConfig = Field(
        default_factory=MapConfig,
        description="Mapping configuration",
    )
    visualize: VisualizeConfig = Field(
        default_factory=VisualizeConfig,
    ...
    num_threads: int = Field(
        default=4,
        ge=1,
        description="Default number of threads for all tools",
    )
    verbose: bool = Field(
        default=False,
